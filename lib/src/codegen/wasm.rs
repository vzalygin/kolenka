...
use crate::{ProgramId, hir::{Hir, HirBaseBlock}, id::{BlockId, VarId}};
...
impl WasmType {
    pub(crate) fn new(params: impl Into<Vec<ValType>>, result: impl Into<Vec<ValType>>) -> WasmType {
        WasmType {
            args: params.into(),
            rets: result.into(),
        }
...
impl WasmModule {
    ...
    pub(crate) fn function_import<'a>(
        &mut self,
    ...
    pub(crate) fn function_export<'a>(
        &mut self,
        id: WasmFunctionId,
        name: impl Into<&'a str>,
    ) {
...
/// Набор предопределенных связок для построения Wasm модуля
#[derive(Debug, Clone)]
pub(crate) struct WasmModuleBundle {
    pub(crate) functions: HashMap<ProgramId, WasmFunctionId>,
}

/// Набор предопределенных связок для построения Wasm функции
#[derive(Debug, Clone)]
pub(crate) struct WasmFunctionBundle {
    pub(crate) blocks: HashMap<BlockId, HirBaseBlock>,
    pub(crate) locals: HashMap<VarId, WasmLocalId>,
    pub(crate) locals_decl: Vec<(u32, ValType)>,
}
